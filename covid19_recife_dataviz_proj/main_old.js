...
let	TS_notloaded = true;
let scatterplot_notloaded = true;
...
function updateBoundaries() {
	...
	scale = d3.scaleQuantize().range(['#B9B3B3', '#FEC5C5', '#FF9494', '#FF5F5F', '#FF3838', '#FF0000']);
    ...
    updateLegend(scale)
    if(TS_notloaded == false){updateTimeSeries()}
}
...
function updateSelectedNodesDiv(node){
	var p = d3.select("#selected_nodes")
	.selectAll("p")
	.data(selected_nodes)
...
function drawGraph_TS(){
	scatterplot_notloaded = true;
	if(TS_notloaded){
		var temp = d3.select("#graphDiv")
	    ...
	    var dropDownMenu = d3.select("#graphDiv").append("div").attr("div","select_div").append("select")
			.attr("id","ValueOption_TS").attr("name","ValueOption_TS").on("change", updateTimeSeries);
	...
	*/
	...
	for (bairro in data[0]){
		if(bairro != 'Date'){
			...
			.style("fill", nodes[bairro].boundary.options.fillColor)
			.text(bairro);
		}
		indx_clr = indx_clr + 1
	}
...
function handleMouseOverTS(d, i) {  // Add interactivity
    ...
    // Use D3 to select element, change color and size
   	...
   	svg.select("#" + this.id + "text").style("font-size",14)
...
function handleMouseOutTS(d, i){
	...
	svg.select("#" + this.id + "text").style("font", "bold 10px sans-serif")
...
function drawScatterPlot(){
	...
	if(scatterplot_notloaded){
	    ...
	    var dropDownMenu = d3.select("#graphDiv").append("div").attr("div","select_div").append("select")
			.attr("id","ValueOption_1").attr("name","ValueOption_TS").on("change", updateScatterPlot);
		d3.select("#ValueOption_1").selectAll("option")            
			...
			if(i == 0)
	            ...
	            .text(d=>d.text)
	    var dropDownMenu = d3.select("#graphDiv").select("div")
		dropDownMenu.append("select").attr("id","ValueOption_2").attr("name","ValueOption_neighborhood").on("change", updateScatterPlot);
